    ...
    while generated.len() < max_new_tokens {
        ...
        for step in 0..config.draft_depth {
            ...
            let last = logits
                ...
                .to_vec1::<f32>()
                .map_err(Error::Candle)?;
            let top: Vec<u32> = top_k_indices(&last, config.top_k_per_step)
                .into_iter()
                .map(|i| i as u32)
                .collect();
            ...
            // draft autoregression). v0.2 will branch the draft state too.
            let next_id = top[0];
        ...
        // 3. Build Cartesian-product tree.
        let tree = crate::methods::medusa::MedusaHeads::from_config(
